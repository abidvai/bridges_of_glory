...
class SearchService {
  Future<ApiResponse<List<ProjectModel>>> searchHome({
    int? categoryID,
    int? projectID,
    ...
    try {
      final response = await CustomHttp.get(
        endpoint:
            // 'projects/list?program_id=$projectID&category_id=${categoryID ?? -101}&search=$searchText',
            'projects/home?search=$searchText',
        needAuth: true,
  ...
  Future<ApiResponse<List<ProjectModel>>> search({
    ...
    try {
      final response = await CustomHttp.get(
        endpoint:
            // 'projects/list?program_id=$projectID&category_id=${categoryID ?? -101}&search=$searchText',
            'projects/list?category_id=${categoryID?.toString() ?? ''}&program_id=$projectID&search=$searchText',
        needAuth: true,
        showFloatingError: false,
      );
